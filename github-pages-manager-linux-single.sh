...
MARKER='__GPM_EMBEDDED_POWERSHELL_PAYLOAD_BELOW__'
...
exit 0
__GPM_EMBEDDED_POWERSHELL_PAYLOAD_BELOW__
#!/usr/bin/env pwsh
# Linux port of github-pages-manager.bat
# Source: original BAT embeds PowerShell after "### POWERSHELL ###".
# This file keeps the PowerShell logic as line-by-line as possible and only patches Windows-only runtime pieces.
...
# Windows APPDATA equivalent for this app on Linux.
$script:GpmAppData = $script:GpmDataRoot

if ([string]::IsNullOrWhiteSpace($env:USERNAME) -and ![string]::IsNullOrWhiteSpace($env:USER)) { $env:USERNAME = $env:USER }
if ([string]::IsNullOrWhiteSpace($env:COMPUTERNAME) -and ![string]::IsNullOrWhiteSpace($env:HOSTNAME)) { $env:COMPUTERNAME = $env:HOSTNAME }
if ([string]::IsNullOrWhiteSpace($env:USERPROFILE)) { $env:USERPROFILE = $script:GpmHome }
...
function Set-ConsoleVisualProfile {
    # Linux terminal font/window sizing is controlled by the terminal emulator, not kernel32.dll.
    # Keep the function name so the original call flow stays the same.
    try {
        $Host.UI.RawUI.WindowTitle = "GitHub Pages Manager"
    }
    catch {
    }
}
...
function Open-External($target) {
    ...
    try {
        if (Get-Command "xdg-open" -ErrorAction SilentlyContinue) {
            & xdg-open ([string]$target) >/dev/null 2>&1
            return
        }
    }
    catch {
    ...
    try {
        Start-Process ([string]$target) | Out-Null
    }
    catch {
        Write-StatusWarn "Acilamadi: $target"
    }
}

function Copy-GpmText($text) {
    try {
    ...
    try {
        if (Get-Command "xclip" -ErrorAction SilentlyContinue) {
            [string]$text | xclip -selection clipboard
            return $true
        }
    }
    catch {
...
$StoreDir = Join-Path $script:GpmAppData "GithubPagesPublisher"
...
$AdminConfigPath = Join-Path (Join-Path $script:GpmAppData "GithubPagesPublisherAdmin") "admin.json"
...
function Invoke-JsonUrl($url) {
    ...
    if ($null -ne $curl) {
        ...
        try {
            $curlOutput = & curl --location --silent --show-error --fail `
...
function Get-LocalAdminManifest {
    $manifestPath = Join-Path (Join-Path (Join-Path $script:GpmAppData "GithubPagesPublisherAdmin") "update-repo") "latest.json"
...
function Get-LocalAdminReleaseRoot {
    return (Join-Path (Join-Path $script:GpmAppData "GithubPagesPublisherAdmin") "update-repo")
}
...
'@.Replace("__CONFIG_B64__", $configB64)
        ...
        Start-Process -FilePath "pwsh" -ArgumentList @("-NoProfile", "-File", $TelemetryWorkerPath) | Out-Null
    }
    catch {
        Remove-Item -LiteralPath $TelemetryWorkerLockPath -Force -ErrorAction SilentlyContinue
...
function Start-SelfUpdate($manifest, $manifestUrl) {
    ...
    $downloadPath = Join-Path $updatesDir "github-pages-manager-$versionSafe.ps1"
    $updaterPath = Join-Path $updatesDir "apply-update.ps1"
    $backupPath = Join-Path $backupDir "github-pages-manager-$AppVersion-$(Get-Date -Format 'yyyyMMdd-HHmmss').ps1"
...
function Start-UpdatedTarget {
    ...
    try {
        Start-Process -FilePath "pwsh" -WorkingDirectory $targetDir -ArgumentList @("-NoProfile", "-File", $Target) | Out-Null
        return
    }
    catch {
    }
    ...
    try {
        if (Get-Command "xdg-open" -ErrorAction SilentlyContinue) {
            & xdg-open $targetDir >/dev/null 2>&1
        }
    }
    catch {
    }
...
function Refresh-Path {
    # Linux PATH is inherited from the shell. Nothing to refresh here.
}
...
function Install-Tool($id, $label) {
    ...
    if (!(Command-Exists "sudo")) {
        throw "sudo bulunamadi. $label otomatik kurulamiyor."
    }
...
function Ensure-Tools {
    ...
    if (!(Command-Exists "git")) {
        throw "Git kuruldu ama bu oturumda gorunmuyor. Terminali yeniden acip tekrar dene."
    }
    ...
    if (!(Command-Exists "gh")) {
        throw "GitHub CLI kuruldu ama bu oturumda gorunmuyor. Terminali yeniden acip tekrar dene."
    }
}
